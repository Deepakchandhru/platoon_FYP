...
# ============================================================
# SYBIL & SPOOFING ATTACK CONFIGURATION
# ============================================================

# ── Sybil Attack ──────────────────────────────────────────────
# One physical vehicle registers multiple fake identities with CA
# then uses them to cast multiple votes, manipulating reputation.
# Detection: CA tracks registrations per real_id; multiple registrations
#            from the same physical vehicle are flagged and blocked.
# Reference: Boeira et al. [1] "On the impact of Sybil attacks
#            in cooperative driving scenarios"

SYBIL_ATTACKER_ID       = "v_p1_5"   # 5th vehicle in platoon 1 is Sybil node
SYBIL_ATTACK_START_TIME = 120.0       # sim-seconds when Sybil identities activate
SYBIL_ATTACK_END_TIME   = 180.0       # sim-seconds when attack ends
SYBIL_NUM_FAKE_IDS      = 3           # number of fake identities created per attacker
SYBIL_VOTE_INTERVAL     = 1.0         # seconds between Sybil vote injections
SYBIL_MAX_REGISTRATIONS = 1           # CA blocks registrations beyond this per real_id

# ── Spoofing Attack ───────────────────────────────────────────
# Vehicle broadcasts fake GPS/position data to mislead platoon
# formation and force other vehicles to react to ghost positions.
# Detection: RSU cross-validates beacon positions against dead-reckoning
#            estimate; large deviations beyond threshold flag spoofing.
# Reference: Junaidi et al. [4] "Secure vehicular platoon management
#            against Sybil attacks"

SPOOFING_ATTACKER_ID        = "v_p2_2"   # 2nd vehicle in platoon 2 is spoofer
SPOOFING_ATTACK_START_TIME  = 160.0      # sim-seconds when spoofing starts
SPOOFING_ATTACK_END_TIME    = 210.0      # sim-seconds when spoofing ends
SPOOFING_POSITION_JUMP      = 150.0      # metres how far the fake position lies
SPOOFING_SPEED_FAKE         = 30.0       # m/s fake speed broadcasted
SPOOFING_INTERVAL           = 0.2        # seconds between spoofed beacons
SPOOFING_DETECTION_THRESHOLD = 15.0      # metres max tolerated position deviation
...
class SybilAttackEngine:
    """
    Models a Sybil attacker that creates multiple fake identities.

    HOW IT WORKS (Section V.B / [1] Boeira et al.):
      At SYBIL_ATTACK_START_TIME the attacker calls ca.register_vehicle()
      SYBIL_NUM_FAKE_IDS times under synthetic real_ids ("v_p1_5_sybil_N").
      Each registration produces a unique fake_id and key pair.
      The attacker then uses these fake identities to inject UPVOTE votes
      for itself, artificially inflating its reputation score.

    DETECTION (SecurityPropertiesValidator.check_sybil):
      CA counts registrations per physical vehicle base ID.
      Any vehicle exceeding SYBIL_MAX_REGISTRATIONS is flagged.
      Detected fake_ids are added to sybil_blacklist; their votes discarded.

    REPUTATION IMPACT:
      Attacker physical ID receives DOWNVOTE each reputation window.
      All fake IDs receive SEVERE_DOWNVOTE and are blacklisted.
    """
    def __init__(self, ca, reputation_manager, security_validator, sec_logger):
        self.ca=ca; self.reputation_manager=reputation_manager
        self.security_validator=security_validator; self.sec_logger=sec_logger
        self.fake_vims=[]; self.fake_ids_created=[]
        self.attack_active=False; self._last_vote_time=0.0
        self._identities_created=False

    def maybe_activate(self, sim_time):
        if (not self._identities_created
                and SYBIL_ATTACK_START_TIME <= sim_time <= SYBIL_ATTACK_END_TIME):
            self._create_fake_identities(sim_time)
            self._identities_created=True; self.attack_active=True
            print("[Sybil] ATTACK STARTED: %s created %d fake IDs @t=%.1fs"%(
                SYBIL_ATTACKER_ID, len(self.fake_vims), sim_time))
            self.sec_logger.log_security(sim_time,"SYBIL_START",SYBIL_ATTACKER_ID,
                "fake_ids=%s"%str([f[:8]+"..." for f in self.fake_ids_created]))
        if sim_time > SYBIL_ATTACK_END_TIME and self.attack_active:
            print("[Sybil] ATTACK ENDED @t=%.1fs"%sim_time)
            self.sec_logger.log_security(sim_time,"SYBIL_END",SYBIL_ATTACKER_ID,"window closed")
            self.attack_active=False

    def _create_fake_identities(self, sim_time):
        for i in range(SYBIL_NUM_FAKE_IDS):
            fake_real_id="%s_sybil_%d"%(SYBIL_ATTACKER_ID,i)
            vim=VehicleIdentityManager(fake_real_id,self.ca)
            reg_ok=vim._register(sim_time)
            if reg_ok:
                result=vim.authenticate_with_ca(sim_time)
                self.security_validator.check_sybil(
                    fake_real_id,SYBIL_ATTACKER_ID,vim.fake_id,result.accepted,sim_time)
                self.fake_vims.append(vim)
                if vim.fake_id: self.fake_ids_created.append(vim.fake_id)

    def inject_fake_votes(self, sim_time, target_vid):
        if not self.attack_active: return
        if (sim_time-self._last_vote_time)<SYBIL_VOTE_INTERVAL: return
        self._last_vote_time=sim_time
        injected=0
        for vim in self.fake_vims:
            if vim.fake_id and self.security_validator.is_sybil_id(vim.fake_id):
                continue
            self.reputation_manager.add_vote(target_vid,VoteType.UPVOTE,False)
            injected+=1
        if injected>0:
            print("[Sybil] Injected %d fake UPVOTEs for %s @t=%.1fs"%(injected,target_vid,sim_time))
            self.sec_logger.log_security(sim_time,"SYBIL_VOTE_INJECT",SYBIL_ATTACKER_ID,
                "%d fake upvotes for %s"%(injected,target_vid))


# ============================================================
# SPOOFING ATTACK ENGINE
# ============================================================

class SpoofingAttackEngine:
    """
    Models GPS/position spoofing: vehicle broadcasts fake coordinates.

    HOW IT WORKS (Section V.B / [4] Junaidi et al.):
      During the attack window the attacker sends BeaconMessages with:
        position = real_position + SPOOFING_POSITION_JUMP  (150m ahead)
        speed    = SPOOFING_SPEED_FAKE                      (30 m/s)
      These are injected into MessageBroker for all nearby vehicles.
      Receivers that are within BEACON_VERIFICATION_RANGE will try to
      verify against kinematic prediction — the 150m jump far exceeds
      the 5m CATS_POSITION_PREDICTION_THRESHOLD and triggers DOWNVOTE.
      The SecurityPropertiesValidator additionally applies SEVERE_DOWNVOTE
      for the confirmed simultaneous presence contradiction.

    DETECTION (SecurityPropertiesValidator.check_spoofing):
      |reported_position - SUMO_real_position| > SPOOFING_DETECTION_THRESHOLD
      Cross-validated every time a spoofed beacon is broadcast.

    REPUTATION IMPACT:
      SEVERE_DOWNVOTE per detected spoofed beacon -> rapid Banned status.
    """
    def __init__(self, trust_vehicle_mgr, message_broker,
                 reputation_manager, security_validator, sec_logger):
        self.trust_vehicle_mgr=trust_vehicle_mgr; self.message_broker=message_broker
        self.reputation_manager=reputation_manager
        self.security_validator=security_validator; self.sec_logger=sec_logger
        self._last_spoof_time=0.0; self._seq_no=0; self._attack_logged=False

    def maybe_spoof(self, sim_time):
        if not(SPOOFING_ATTACK_START_TIME<=sim_time<=SPOOFING_ATTACK_END_TIME): return
        if (sim_time-self._last_spoof_time)<SPOOFING_INTERVAL: return
        real_state=self.trust_vehicle_mgr.get_vehicle_state(SPOOFING_ATTACKER_ID)
        if real_state is None: return
        self._last_spoof_time=sim_time; self._seq_no+=1
        if not self._attack_logged:
            print("[Spoof] ATTACK STARTED: %s fake pos=real+%.0fm @t=%.1fs"%(
                SPOOFING_ATTACKER_ID,SPOOFING_POSITION_JUMP,sim_time))
            self.sec_logger.log_security(sim_time,"SPOOF_START",SPOOFING_ATTACKER_ID,
                "real_pos=%.1f fake=%.1f jump=%.1fm"%(
                    real_state['position'],
                    real_state['position']+SPOOFING_POSITION_JUMP,
                    SPOOFING_POSITION_JUMP))
            self._attack_logged=True
        spoofed=BeaconMessage(
            sender_id=SPOOFING_ATTACKER_ID,
            timestamp=sim_time,
            seq_no=self._seq_no,
            lane=real_state['lane'],
            position=real_state['position']+SPOOFING_POSITION_JUMP,
            speed=SPOOFING_SPEED_FAKE,
            acceleration=0.0)
        self.message_broker.broadcast_message(
            spoofed,real_state['position'],self.trust_vehicle_mgr.vehicle_states)
        self.security_validator.check_spoofing(
            SPOOFING_ATTACKER_ID,
            real_state['position'],spoofed.position,
            spoofed.speed,real_state['speed'],
            sim_time,self.reputation_manager)


# ── Extend SecurityPropertiesValidator with Sybil + Spoofing methods ─────────

_orig_spv_init=SecurityPropertiesValidator.__init__
def _spv_init2(self):
    _orig_spv_init(self)
    self.sybil_events=[]; self.sybil_blacklist=set()
    self._real_id_reg_count={}
    self.spoofing_events=[]; self.spoofing_blacklist=set()
SecurityPropertiesValidator.__init__=_spv_init2

def _check_sybil(self, fake_real_id, attacker_phys_id, fake_id, was_accepted, sim_time):
    """
    Detect Sybil attack: same physical vehicle registers multiple CA identities.

    Detection strategy:
      Count registrations per physical base ID.
      Exceeding SYBIL_MAX_REGISTRATIONS => Sybil detected.
      Detected fake_id added to sybil_blacklist => its votes discarded.

    Paper reference: Boeira et al. [1] "effects of colluding Sybil nodes
    in message falsification attacks for vehicular platooning" [3].
    """
    count=self._real_id_reg_count.get(attacker_phys_id,0)+1
    self._real_id_reg_count[attacker_phys_id]=count
    is_sybil=count>SYBIL_MAX_REGISTRATIONS
    entry={'sim_time':sim_time,'fake_real_id':fake_real_id,
           'attacker_phys_id':attacker_phys_id,'fake_id':fake_id,
           'was_accepted':was_accepted,'registration_count':count,'detected':is_sybil}
    self.sybil_events.append(entry)
    if is_sybil and fake_id:
        self.sybil_blacklist.add(fake_id)
        print("[SECURITY] SYBIL DETECTED: %s reg#%d fake_id=%s... BLACKLISTED"%(
            attacker_phys_id,count,fake_id[:12]))
    return is_sybil
SecurityPropertiesValidator.check_sybil=_check_sybil

def _is_sybil_id(self,fake_id): return fake_id in self.sybil_blacklist
SecurityPropertiesValidator.is_sybil_id=_is_sybil_id

def _check_spoofing(self, vid, real_pos, reported_pos, reported_speed,
                     real_speed, sim_time, reputation_manager):
    """
    Detect GPS/position spoofing in a beacon message.

    Detection logic (three signals):
      1. Position deviation: |reported - real| > SPOOFING_DETECTION_THRESHOLD (15m)
         Much larger than the 5m kinematic threshold to avoid false positives.
      2. Impossible speed: reported_speed > CATS_MAX_SPEED (33.3 m/s)
      3. Simultaneous presence contradiction: vehicle appears at two positions.

    On detection:
      - SEVERE_DOWNVOTE applied directly (same weight as false obstacle attack)
      - vid added to spoofing_blacklist
      - SPOOF_DETECTED logged to security CSV

    Paper reference: Junaidi et al. [4] "Secure vehicular platoon management
    against Sybil attacks" — position spoofing detection via cross-validation.
    """
    deviation=abs(reported_pos-real_pos)
    speed_bad=reported_speed>CATS_MAX_SPEED
    is_spoofing=(deviation>SPOOFING_DETECTION_THRESHOLD or speed_bad)
    if is_spoofing:
        self.spoofing_events.append({'sim_time':sim_time,'vehicle_id':vid,
            'real_pos':real_pos,'reported_pos':reported_pos,
            'deviation_m':deviation,'real_speed':real_speed,
            'reported_speed':reported_speed,'speed_impossible':speed_bad})
        self.spoofing_blacklist.add(vid)
        reputation_manager.add_vote(vid,VoteType.SEVERE_DOWNVOTE,False)
        print("[SECURITY] SPOOFING DETECTED: %s reported=%.1f real=%.1f dev=%.1fm spd=%.1f->%.1f @t=%.1fs"%(
            vid,reported_pos,real_pos,deviation,real_speed,reported_speed,sim_time))
    return is_spoofing
SecurityPropertiesValidator.check_spoofing=_check_spoofing

def _is_spoofing(self,vid): return vid in self.spoofing_blacklist
SecurityPropertiesValidator.is_spoofing=_is_spoofing

_orig_print_summary=SecurityPropertiesValidator.print_summary
def _print_summary2(self):
    _orig_print_summary(self)
    print("="*70)
    print("SYBIL ATTACK SUMMARY")
    print("="*70)
    total_sybil=len([e for e in self.sybil_events if e['detected']])
    print("  Registration attempts : %d"%len(self.sybil_events))
    print("  Sybil detections      : %d"%total_sybil)
    print("  Blacklisted fake IDs  : %d"%len(self.sybil_blacklist))
    if self.sybil_events:
        print("  Attacker vehicle      : %s"%SYBIL_ATTACKER_ID)
        print("  Attack window         : %.1fs - %.1fs"%(SYBIL_ATTACK_START_TIME,SYBIL_ATTACK_END_TIME))
    print("")
    print("="*70)
    print("SPOOFING ATTACK SUMMARY")
    print("="*70)
    print("  Spoofed beacons detected   : %d"%len(self.spoofing_events))
    print("  Spoofing vehicles flagged  : %d"%len(self.spoofing_blacklist))
    if self.spoofing_events:
        devs=[e['deviation_m'] for e in self.spoofing_events]
        print("  Max position deviation     : %.1fm"%max(devs))
        print("  Avg position deviation     : %.1fm"%(sum(devs)/len(devs)))
        print("  Attacker vehicle           : %s"%SPOOFING_ATTACKER_ID)
        print("  Attack window              : %.1fs - %.1fs"%(SPOOFING_ATTACK_START_TIME,SPOOFING_ATTACK_END_TIME))
    print("="*70+"\n")
SecurityPropertiesValidator.print_summary=_print_summary2
...
def main():
    ...
    _sybil_engine_ref   = [None]
    _spoofing_engine_ref = [None]
    ...
    _sybil_engine_ref[0] = SybilAttackEngine(
        ca, reputation_manager, security_validator, sec_logger)
    _spoofing_engine_ref[0] = SpoofingAttackEngine(
        trust_vehicle_mgr, message_broker,
        reputation_manager, security_validator, sec_logger)
    sybil_engine    = _sybil_engine_ref[0]
    spoofing_engine = _spoofing_engine_ref[0]
    ...
    safe_put(msg_q, "  [Sybil]  Attacker: %s  window: %.0fs-%.0fs" % (SYBIL_ATTACKER_ID,SYBIL_ATTACK_START_TIME,SYBIL_ATTACK_END_TIME))
    safe_put(msg_q, "  [Spoof]  Attacker: %s  window: %.0fs-%.0fs" % (SPOOFING_ATTACKER_ID,SPOOFING_ATTACK_START_TIME,SPOOFING_ATTACK_END_TIME))
...
#  may not be entertained" -> stale key = impersonation flag
            ...
            # Sybil: create fake identities at attack start time
            sybil_engine.maybe_activate(sim_time)
            # Sybil: inject fake votes using undetected identities
            if (SYBIL_ATTACK_START_TIME<=sim_time<=SYBIL_ATTACK_END_TIME
                    and SYBIL_ATTACKER_ID in present):
                sybil_engine.inject_fake_votes(sim_time,SYBIL_ATTACKER_ID)
            # Spoofing: broadcast false position beacons
            spoofing_engine.maybe_spoof(sim_time)
            ...
            for receiver_id in list(present):
                ...
                for message in message_broker.get_inbox(receiver_id):
                    # Discard votes from Sybil-blacklisted fake IDs
                    _svim=vehicle_identity_managers.get(message.sender_id)
                    if (_svim and _svim.fake_id
                            and security_validator.is_sybil_id(_svim.fake_id)):
                        continue
                    # Discard votes from detected spoofers
                    if security_validator.is_spoofing(message.sender_id):
                        continue
            ...
            if (sim_time - cats_window_start_time
                ...
                for veh_id in list(cats_initialized_vehicles):
                    # Sybil penalty: physical attacker gets DOWNVOTE
                    if (veh_id==SYBIL_ATTACKER_ID
                            and security_validator.sybil_events
                            and SYBIL_ATTACK_START_TIME<=sim_time<=SYBIL_ATTACK_END_TIME):
                        reputation_manager.add_vote(veh_id,VoteType.DOWNVOTE,False)
    ...
    # ==========================================================
        ...
        sec_logger.flush()
        if spoofing_engine._attack_logged:
            sec_logger.log_security(sim_time,"SPOOF_END",SPOOFING_ATTACKER_ID,"sim ended")
            sec_logger.flush()
