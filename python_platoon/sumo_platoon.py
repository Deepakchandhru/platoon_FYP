...
# --- ADDED: bring in auth_server register/auth logic ---
import grpc
...
MERGE_TARGET_DIST = 33.0       
MERGE_TOLERANCE = 0.75           
MERGE_SLOW_REGION = 120.0         

SIDE1_FORCE_SPAWN_STEP = 1245
...
def rsu_auth_and_join(_, plate, pid=None, rsu_id: Optional[str]=None):
    ...
    # Fallback: perform registration using NODE_URL and return commitment as pid
    prefix = plate.split("-")[0]
    if rsu_id:
        safe_put(msg_q, f"[RSU {rsu_id}] auth/join request for {plate}")

    vehicle_secret = plate  # using plate as a simple secret in this fallback
    manufacturer_signature = f"sig-{plate}"
    ...
    # if registration failed, return a mock pid based on plate
...
def main():
    ...
    for vidx in range(PLATOON_SIZE):
        vid = vid_for(0, vidx)
        ...
        while step < SIM_STEPS:
            ...
            try:
                if (not merged_done) and (not merge_attempted):
                    p1_tail = vid_for(0, PLATOON_SIZE-1)   
                    p2_head = vid_for(1, 0)             
                    if p1_tail in present and p2_head in present:
                        ...
                        if step >= SLOW_START_STEP and tail_to_head > (MERGE_TARGET_DIST + MERGE_TOLERANCE):
                            ...
                            if gap_excess > 0:
                                ...
                                safe_put(msg_q, f"[PLATOON] slowing Platoon-2 head to close gap ({tail_to_head:.1f}m -> target {MERGE_TARGET_DIST}m)")
                        ...
                        if step >= MERGE_EXECUTE_STEP and tail_to_head <= (MERGE_TARGET_DIST + MERGE_TOLERANCE):
                            ...
                            safe_put(msg_q, f"[PLATOON] proximity detected tail {p1_tail} <-> head {p2_head} dist={tail_to_head:.1f} -> requesting merge")
                            ...
                            if resp is None:
                                resp = do_merge(None, leader_plate, pids.get(0), pids.get(1))
                                ...
                                for v in merged_vids:
                                    if v not in vid_confidence:
                                        try:
                                            ...
                                            vid_confidence[v] = 0.0
                                new_leader = bully_elect_leader(merged_vids) or p1_tail
                                try:
                                    view = get_view_id()
                                    if view:
                                        traci.gui.trackVehicle(view, new_leader)
                                    traci.vehicle.setColor(new_leader, (255, 240, 0, 255))
                                except Exception:
                                    pass
                                for v in merged_vids:
                                    vid_platoon_index[v] = 0
                                    vid_to_pid[v] = pids.get(0)
                                synchronize_platoon_to_leader(merged_vids, new_leader)
                                merged_done = True
                                safe_put(msg_q, f"[PLATOON] merged (proximity): elected leader {new_leader}, synced {len(merged_vids)} vehicles")
                            else:
            ...
            if step == SIDE1_ARRIVE_STEP and not merge_attempted:
                ...
                if p1_tail in traci.vehicle.getIDList() and p2_head in traci.vehicle.getIDList():
                    ...
                    if step < MERGE_EXECUTE_STEP:
                        ...
                        if step >= SLOW_START_STEP and gap_excess > 0:
                            _slowdown_platoon2_towards(leader_speed, gap_excess)
                        safe_put(msg_q, f"[FORCED] Step {step}: merge deferred until step {MERGE_EXECUTE_STEP} - slowing Platoon-2 head (gap {tail_to_head:.1f}m > {MERGE_TARGET_DIST}m)")
                    else:
                        safe_put(msg_q, f"[FORCED] Step {step}: initiating merge of Platoon-2 into main platoon (dist={tail_to_head:.1f}m)")
                        if pids.get(0) is None:
                            pids[0] = rsu_auth_and_join(None, plate_for(0,0))
                        if pids.get(1) is None:
                            pids[1] = rsu_auth_and_join(None, plate_for(1,0))
                        ...
                        if resp and getattr(resp, "ok", False):
                            try:
                                try:
                                    main_route_edges = traci.route.getEdges("r_main") if "r_main" in traci.route.getIDList() else main_edges[:]
                                except Exception:
                                    main_route_edges = main_edges[:]
                                if not main_route_edges:
                                    safe_put(msg_q, "[PLATOON] FORCED merge failed: no main route edges available")
                                else:
                                    merged_vids = []
                                    for i in range(PLATOON_SIZE):
                                        vid2 = vid_for(1, i)
                                        if vid2 in traci.vehicle.getIDList():
                                            try:
                                                target_edges = pick_merge_edges_for_vid(vid2, main_route_edges, preferred_main="main_1")
                                                if not safe_set_vehicle_route(vid2, target_edges):
                                                    for e in target_edges:
                                                        try:
                                                            traci.vehicle.changeTarget(vid2, e)
                                                            break
                                                        except Exception:
                                                            continue
                                                ...
                                                try:
                                                    ...
                                                    leader_speed = LEADER_SPEED_MPS
                                                traci.vehicle.slowDown(vid2, leader_speed, 2.0)
                                                try:
                                                    if vid_for(0,0) in traci.vehicle.getIDList():
                                                        lane_idx = traci.vehicle.getLaneIndex(vid_for(0,0))
                                                        traci.vehicle.changeLane(vid2, lane_idx, 2.0)
                                                except Exception:
                                                    pass
                                                vid_platoon_index[vid2] = 0
                                                merged_vids.append(vid2)
                                            except Exception:
                                                try:
                                                    traci.vehicle.changeTarget(vid2, main_route_edges[0])
                                                    vid_platoon_index[vid2] = 0
                                                    merged_vids.append(vid2)
                                                except Exception as e:
                                                    safe_put(msg_q, f"[PLATOON] failed to route {vid2} into main: {e}")
                                    for v in traci.vehicle.getIDList():
                                        if vid_platoon_index.get(v) == 0 and v not in merged_vids:
                                            merged_vids.append(v)
                                    if merged_vids:
                                        for v in merged_vids:
                                            if v not in vid_confidence:
                                                try:
                                                    ...
                                                    vid_confidence[v] = 0.0
                                        new_leader = bully_elect_leader(merged_vids) or vid_for(0,0)
                                        for v in merged_vids:
                                            vid_to_pid[v] = pids.get(0)
                                            vid_platoon_index[v] = 0
                                        synchronize_platoon_to_leader(merged_vids, new_leader)
                                        merged_done = True
                                        merged_at = sim_time
                                        safe_put(msg_q, f"[PLATOON] FORCED merge executed: elected {new_leader}, synced {len(merged_vids)} vehicles")
                            except Exception as e:
                                safe_put(msg_q, f"[PLATOON] FORCED merge execution failed: {e}")
                        else:
                            safe_put(msg_q, f"[PLATOON] FORCED merge rejected by server: {getattr(resp,'message',None)}")
            ...
            if step == 2300 and not middle_left_done:
                try:
                    ...
                    for pidx in range(PLATOON_COUNT):
                        mid_vid = vid_for(pidx, 1)
                        if mid_vid in traci.vehicle.getIDList():
                            chosen_vid = mid_vid
                            chosen_pidx = pidx
                            break
                    if chosen_vid:
                        leave_resp = _call_grpc_fn(["do_leave", "leave", "leave_platoon", "request_leave", "sumo_leave"], plate_for(chosen_pidx, 1))
                        if leave_resp is None:
                            try:
                                do_leave(None, plate_for(chosen_pidx, 1))
                            except Exception:
                                pass
            ...
            if step % 50 == 0:
                ...
                for pidx in range(PLATOON_COUNT):
                        dists = []
                        members = [vid_for(pidx, i) for i in range(PLATOON_SIZE) if vid_for(pidx, i) in ids]
