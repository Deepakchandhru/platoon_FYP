...
try:
    from dashboard_integration import attach_dashboard, update_sim_time as _dash_update_time
    _DASHBOARD_ENABLED = True
except ImportError:
    _DASHBOARD_ENABLED = False
    def _dash_update_time(*a, **k): pass
    print("[Dashboard] dashboard_integration.py not found — dashboard disabled")
...
CATS_BLOCKCHAIN_INIT_SCORE = 80.0   
...
SYBIL_ATTACKER_ID       = "v_p1_5"   
SYBIL_ATTACK_START_TIME = 120.0       
SYBIL_ATTACK_END_TIME   = 180.0      
SYBIL_NUM_FAKE_IDS      = 3           
SYBIL_VOTE_INTERVAL     = 1.0         
SYBIL_MAX_REGISTRATIONS = 1           

SPOOFING_ATTACKER_ID        = "v_p2_2"   
SPOOFING_ATTACK_START_TIME  = 160.0     
SPOOFING_ATTACK_END_TIME    = 210.0     
SPOOFING_POSITION_JUMP      = 150.0     
SPOOFING_SPEED_FAKE         = 30.0      
SPOOFING_INTERVAL           = 0.2        
SPOOFING_DETECTION_THRESHOLD = 15.0     
    ...
    def apply_trust_based_actions(self, veh_id, trust_state):
        ...
        try:
            ...
            if trust_state == TrustState.TRUSTED:
                ...
                self.pending_banned_leave.add(veh_id)
                self.applied_actions[veh_id] = TrustState.BANNED
                print(f"[CATS] {veh_id} BANNED → queued for platoon-leave")
        except Exception:
            pass
...
def _apply_common_platoon_color(vids, colour):
    print(vids, colour)
    if not vids or colour is None:
        return
    ...
    try:
        while step < SIM_STEPS:
            ...
            if (sim_time - cats_window_start_time
                ...
                _dash_update_time(reputation_manager, sim_time)  
            ...
            if trust_vehicle_mgr.pending_banned_leave:
                for banned_vid in list(trust_vehicle_mgr.pending_banned_leave):
                    try:
                        present_ids = list(traci.vehicle.getIDList())
                        if banned_vid not in present_ids:
                            trust_vehicle_mgr.pending_banned_leave.discard(banned_vid)
                            continue

                        banned_pidx = vid_platoon_index.get(banned_vid)

                        leave_plate = vid_to_plate.get(banned_vid, banned_vid)
                        ...
                        ejected_lane = 3
                        try:
                            traci.vehicle.changeLane(banned_vid, ejected_lane, 2.0)
                        except Exception:
                            try:
                                ejected_lane = 1
                                traci.vehicle.changeLane(banned_vid, ejected_lane, 2.0)
                                ...
                                pass
                        try:
                            traci.vehicle.setLaneChangeMode(banned_vid, 0)
                        except Exception:
                            pass

                        try:
                            traci.vehicle.slowDown(
                                banned_vid, float(CATS_BANNED_SPEED), 3.0)
                        except Exception:
                            try:
                                traci.vehicle.setSpeed(
                                    banned_vid, float(CATS_BANNED_SPEED))
                            except Exception:
                                pass

                        if banned_pidx is not None and banned_pidx >= 0:
                            members = [v for v in present_ids
                                       if vid_platoon_index.get(v) == banned_pidx]
                            try:
                                members.sort(
                            ...
                            if banned_vid in members:
                                b_idx  = members.index(banned_vid)
                                ahead  = members[:b_idx]
                                behind = members[b_idx + 1:]
                            else:
                                ahead  = []
                                behind = [v for v in members if v != banned_vid]

                            new_idx = _find_free_platoon_index(
                                pids, vid_platoon_index, present_ids)
                            if new_idx == banned_pidx:
                                new_idx = banned_pidx + 1

                            for v in ahead:
                                vid_platoon_index[v] = banned_pidx
                            for v in behind:
                            ...
                            vid_platoon_index[banned_vid] = -1
                            vid_to_pid[banned_vid]        = None

                            try:
                                traci.vehicle.setColor(
                                    banned_vid, (80, 80, 80, 255))
                                colored.add(banned_vid)
                            except Exception:
                                pass
                            try:
                                for v in ahead:
                                    traci.vehicle.setColor(
                                        v, PLATOON_COLORS[banned_pidx % len(PLATOON_COLORS)])
                                    colored.add(v)
                            except Exception:
                                pass
                            try:
                                for v in behind:
                                    traci.vehicle.setColor(
                                        v, PLATOON_COLORS[new_idx % len(PLATOON_COLORS)])
                                    colored.add(v)
                            except Exception:
                                pass
                            ...
                            try:
                                _call_grpc_fn(
                                    ["notify_platoon_split", "split_platoon",
                                     "platoon_split"],
                                    banned_pidx, new_idx, ahead, behind)
                            except Exception:
                                pass

                            safe_put(msg_q,
                                     f"[BANNED-LEAVE] {banned_vid} ejected from "
                                     f"platoon {banned_pidx} → lane {ejected_lane} "
                                     f"locked (no return). Ahead={len(ahead)} "
                                     f"Behind={len(behind)} → new platoon idx {new_idx}")
                        else:
                            safe_put(msg_q,
                                     f"[BANNED-LEAVE] {banned_vid} ejected "
                                     f"(not in any platoon) → lane {ejected_lane} locked")

                        trust_vehicle_mgr.pending_banned_leave.discard(banned_vid)

                    except Exception as _ban_ex:
                        safe_put(msg_q,
                                 f"[BANNED-LEAVE] error ejecting {banned_vid}: {_ban_ex}")
                        trust_vehicle_mgr.pending_banned_leave.discard(banned_vid)
        ...
        if blockchain_mgr.enabled:
            ...
            for veh_id in sorted(cats_initialized_vehicles):
                commitment = vid_to_commitment.get
                (veh_id, "")
                if not commitment:
                    continue
